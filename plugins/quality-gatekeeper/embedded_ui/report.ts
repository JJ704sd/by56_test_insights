...
(() => {
  ...
  const state = { report: null, selectedId: null, full: false };
  let requestId = 0;
  ...
  const rpc = (method, params) => {
    window.parent.postMessage({ jsonrpc: "2.0", id: `qei-${++requestId}`, method, params }, "*");
  };
  ...
  function askSelected() {
    const context = getContext();
    if (!context) return;
    rpc("ui/update-model-context", { content: [{ type: "text", text: JSON.stringify(context) }] });
    ...
    else rpc("ui/message", { content: [{ type: "text", text: prompt }] });
  }
  ...
  window.addEventListener("message", (event) => {
    ...
    const report = message.params?._meta?.qualityReport ?? message.params?.result?._meta?.qualityReport;
  ...
  if (fixtures) {
    ...
    const report = window.openai?.toolResponseMetadata?.qualityReport;
    if (report) render(report);
    rpc("ui/initialize", { clientInfo: { name: "quality-evidence-inspector", version: "1.0.0" } });
  }
})();
